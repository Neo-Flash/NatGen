from natgen.molecule.features import (
    atom_to_feature_vector,
    atom_to_feature_vector_chiral,
...
def rdk2graph(mol: Mol):
    ...
    num_atoms = len(x)
    full_edges_list = list(itertools.combinations(range(num_atoms), 2))
    full_edges_list = full_edges_list + [(j, i) for (i, j) in full_edges_list] 
    full_edge_index = np.array(full_edges_list, dtype=np.int64).T
    graph = dict()
    graph["edge_index"] = edge_index
    graph["edge_attr"] = edge_attr
    graph["node_feat"] = x
    ...
    graph["full_edge_index"] = full_edge_index
    return graph
def rdk2graph_chiral(mol: Mol):
    import itertools
    atom_features_list = []
    for atom in mol.GetAtoms():
        atom_features_list.append(atom_to_feature_vector_chiral(atom))
    x = np.array(atom_features_list, dtype=np.int64)
    num_bond_features = 3
    if len(mol.GetBonds()) > 0:
        edges_list = []
