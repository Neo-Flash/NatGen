...
def isomorphic_core_from_graph(node_features, edge_index, edge_attributes):
    try:
        G = gt.Graph(directed=False)
        G.add_edge_list(np.transpose(edge_index))
        vprop = G.new_vertex_property("vector<float>") 
        for i, feature in enumerate(node_features):
            vprop[G.vertex(i)] = feature
        G.vertex_properties["node_features"] = vprop
        maps = topology.subgraph_isomorphism(
            G, G, 
            vertex_label=(G.vertex_properties["node_features"], G.vertex_properties["node_features"]),
            subgraph=False,
        )
        return [np.array(m.a) for m in maps]
    except:
         return [np.arange(G.num_vertices())]
